...
const STATUS_COPY = {
  400: { title: 'Bad request', message: 'That request was malformed. Please check your input and try again.' },
  401: { title: 'Sign in required', message: 'Your session has expired or you are not signed in.' },
  ...
  502: { title: 'Bad gateway', message: 'The upstream service is unavailable right now.' },
  503: { title: 'Service unavailable', message: 'The service is temporarily down for maintenance.' },
  504: { title: 'Gateway timeout', message: 'The request took too long to respond. Please try again.' }
};
...
export function AuthError({ code = 401 }) {
  return <ErrorPage code={code} />;
}

/** Convenience wrapper for server-side failures (500/502/503/504). */
export function ServerError({ code = 500 }) {
  return <ErrorPage code={code} />;
}
